...
export default function EtfDashboard() {
  ...
  async function loadQuotes() {
    try {
      const data = await api.etfQuotes();
  ...
  async function handleRecommend(ticker) {
    setError("");
    try {
      const data = await api.etfRecommend(ticker);
      setResult(data);
    } catch (err) {
      setError(err.message);
    }
  ...
  async function handleTrack(e) {
    ...
    try {
      await api.etfTrack(newTicker.trim());
      setNewTicker("");
      loadQuotes();
    } catch (err) {
      setError(err.message);
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <h1 className="page-title">ETF Ledger</h1>
        <p className="page-note">
          Live ETF prices, refreshed hourly. Select a fund to generate recommendations
      ...
      <form className="inline-form" onSubmit={handleTrack}>
        <input
          placeholder="ADD ETF TICKER (e.g. VWCE.DE)"
          value={newTicker}
