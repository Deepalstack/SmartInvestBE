...
tr_stocks.py
------------
Trains BOTH XGBoost and Gaussian Process Regressor models on live stock
data pulled via yfinance. Run once (or periodically) before using the
Stocks recommendation endpoint.

Output: xgb_stock_model.joblib, gpr_stock_model.joblib, stock_scaler.joblib
"""
...
from news_fetcher import DEFAULT_STOCK_TICKERS
...
def build_dataset(tickers: list[str]) -> pd.DataFrame:
    ...
    if not frames:
        raise RuntimeError("No data fetched for any ticker — check network/tickers.")

    combined = pd.concat(frames, ignore_index=True)
    combined = combined.dropna(subset=FEATURE_COLS + [TARGET_COL])
    print(f"✅ Built dataset: {len(combined)} rows across {len(frames)} tickers.")
    return combined
...
def main():
    ...
    joblib.dump(gpr_model, "gpr_stock_model.joblib")
    joblib.dump(scaler, "stock_scaler.joblib")
    print("\n✅ Saved: xgb_stock_model.joblib, gpr_stock_model.joblib, stock_scaler.joblib")
